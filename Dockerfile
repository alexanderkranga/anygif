...
RUN dnf install -y tar xz && \
    ...
    tar xJ --strip-components=1 -C /usr/local/bin/ --wildcards '*/ffmpeg' --wildcards '*/ffprobe' && \
    dnf clean all

COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt yt-dlp==2026.3.13 curl_cffi
