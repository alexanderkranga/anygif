...
- **Ephemeral session data only** — Redis stores session context (user_id, chat_id, video_url) for the payment-to-delivery handoff between Lambdas, with a 2-minute TTL. An additional `refund:{session_id}` key (1-hour TTL, deleted on use) holds only `user_id` as a refund fallback. No long-term user data storage exists anywhere.
